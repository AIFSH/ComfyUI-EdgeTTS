...
function previewAudio(node,file,type){
    ...
    previewWidget.trackEl = document.createElement("track");
    previewWidget.trackEl.src = api.apiURL('/view?' + new URLSearchParams({
        "filename": file.split(".")[0] + ".vtt",
        "type": type,
    }));
    previewWidget.trackEl.kind = "subtitles"
    previewWidget.trackEl.hidden = false; 

    previewWidget.audioEl.appendChild(previewWidget.trackEl)
